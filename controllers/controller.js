...
function createRocket() {
    ...
    if (checkRocketName(name)) {
        ...
        document.getElementById("rocketsCreated").innerHTML = "You have created " + rockets.length + " rockets. <br>";
        for (var i = 0; i < rockets.length; i++) {
            document.getElementById("rocketsCreated").innerHTML += "Rocket " + (i + 1) + ": " + x[i].name + "<br>";
        }
    }
    else {
        if (name == "") {
...
//Validación que el nombre tenga 8 caracteres.-
...
function findRocket(RocketToFind1, RockerToFind2) {
    var indexOfFound;
    if (rockets.length === 0) {
...
document.getElementById("speed2").addEventListener("click", function () {
    ...
    if (indexOfFound !== undefined) {
        console.log("Rocket2 ready to accelerate");
        if (rockets[indexOfFound].Poten_Act < calcularPotenciaMaxima(rockets[indexOfFound])) {
            rockets[indexOfFound].speed();
            calcularPotenciaActual(rockets[indexOfFound]);
            console.log("Speed Rocket1 complet!");
